package com.algubra.volleymanager;
...
import com.algubra.constants.ParamConstants;
...
/**
 * Created by gayathri on 18/03/16.
 */
public class VolleyAPIManager implements ParamConstants,URLConstants {
    ...
    public VolleyAPIManager(String requestUrl) {
    ...
    public void getResponseGET(final Context activity, int type, String[] name,
        ...
        for (int i = 0; i < value.length; i++) {
            System.out.println("name getResponseGET" + i + " " + name[i]);
            System.out.println("value getResponseGET" + i + " " + value[i]);
        }
        if (AppUtilityMethods.isNetworkConnected(activity)) {
            ...
            ((Activity) activity).runOnUiThread(new Runnable() {
                ...
                public void run() {
                   /* CustomDialog customDialog = new CustomDialog
                            (activity,"Alert41",activity.getResources().getString(R.string.no_internet));
                    customDialog.show();*/
                }
            });
        }
    ...
    public void getResponsePOST(final Context activity, int type, String[] name,
        ...
        HttpsTrustManager.allowAllSSL();
        for (int i = 0; i < value.length; i++) {
            System.out.println("name getResponsePOST" + i + " " + name[i]);
            System.out.println("value from app getResponsePOST" + i + " " + value[i]);
        }
        if (AppUtilityMethods.isNetworkConnected(activity)) {
            this.mResponseListener = responseListener;
            ...
            if (type == 0)
                ...
                try {
                    ...
                    e.printStackTrace();
                    Log.d(TAG,"Excption in calling getJsonResponsePostCustom");
                }
            }
            else if(type == 8){
                try {
                    getJsonResponsePostCustom1((Activity) activity, name, value);
                }catch (Exception e){
                    e.printStackTrace();
                    Log.d(TAG,"Excption in calling getJsonResponsePostCustom");
                }
            ...
            ((Activity) activity).runOnUiThread(new Runnable() {
                ...
                public void run() {
                   /* CustomDialog customDialog = new CustomDialog
                            (activity, activity.getResources().getString(R.string.error_heading),activity.getResources().getString(R.string.no_internet));
                    customDialog.show();*/
                }
            });
        }
    ...
    public void getJsonResponsePostCustom(final Activity activity, final String[] name,
            ...
            final CustomProgressBar pDialog = new CustomProgressBar(activity,
                    R.drawable.spinner);
        if (!(mRequestUrl.equalsIgnoreCase(URL_GET_BADGE))) {
    ...
    public void getJsonResponsePostCustom1(final Activity activity, final String[] name,
                                          final String[] value) {
