...
import com.algubra.activity.videos.adapter.VideosListAdapter;
 ...
 */
public class VideosListActivity extends Activity implements URLConstants,JsonTagConstants,StausCodes {
    RecyclerView messageList;
    RelativeLayout relativeHeader;
    HeaderManager headermanager;
    ImageView back;
    Context mContext = this;
    ArrayList<VideosModel> listTimetable = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (AppUtilityMethods.isNetworkConnected(mContext)) {
            getTimeTableList(URL_GET_VIDEOS);
        } else {
                ...
                try {
                    ...
                    if (response_code.equalsIgnoreCase(RESPONSE_SUCCESS)) {
                        ...
                        if (status_code.equalsIgnoreCase(STATUSCODE_SUCCESS)) {
                            ...
                            JSONArray staffArray = secobj.getJSONArray(JTAG_VIDEOS);
                            ...
                            if (staffArray.length() > 0) {
                                ...
                                for (int i = 0; i < staffArray.length(); i++) {
                                    ...
                                    VideosModel studentAwardModel = new VideosModel();
                                    studentAwardModel.setId(sObject.optString(JTAG_ID));
                                    studentAwardModel.setTitle(sObject.optString(JTAG_TITLE));
                                    studentAwardModel.setVideo_link(sObject.optString(JTAG_VIDEO_URL));
                                    listTimetable.add(studentAwardModel);
/*        "id": "3",
        "boardid": "1",
        "video_url": "https:\/\/www.youtube.com\/watch?v=aiwLSm9KHUI",
        "title": "Test1",
        "board": "ISG"*/
                                }

                                VideosListAdapter timeTableAdapter = new VideosListAdapter(mContext, listTimetable);
    ...
    private void initUI() {
        ...
        headermanager = new HeaderManager(VideosListActivity.this, "Videos");
        if (AppPreferenceManager.getSchoolSelection(mContext).equals("ISG")) {
        ...
        messageList.addOnItemTouchListener(new RecyclerItemListener(getApplicationContext(), messageList,
                new RecyclerItemListener.RecyclerTouchListener() {
                    public void onClickItem(View v, int position) {
                        ...
                        if (!(listTimetable.get(position).getVideo_link().equalsIgnoreCase(""))) {
                            stopDisconnectTimer();
//                            startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse(listTimetable.get(position).getVideo_link())));
                            Intent intent = new Intent(VideosListActivity.this, OpenYouTubePlayerVideoActivity.class);
                            intent.putExtra("position", position);
                            intent.putExtra("array", listTimetable);
    ...
    private Runnable disconnectCallback = new Runnable() {
        ...
        public void run() {
            ...
            AppPreferenceManager.setIsGuest(VideosListActivity.this, false);
            AppPreferenceManager.setUserId(VideosListActivity.this, "");
            AppPreferenceManager.setIsUserAlreadyLoggedIn(VideosListActivity.this, false);
            AppPreferenceManager.setSchoolSelection(VideosListActivity.this, "ISG");
            if (AppUtilityMethods.isAppInForeground(mContext)) {

                Intent mIntent = new Intent(VideosListActivity.this, LoginActivity.class);
