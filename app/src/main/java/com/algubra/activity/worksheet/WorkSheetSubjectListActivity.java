...
import com.algubra.activity.worksheet.adapter.WorksheetSubjectListAdapter;
...
public class WorkSheetSubjectListActivity extends Activity implements URLConstants, JsonTagConstants, StausCodes {
    RecyclerView messageList;
    RelativeLayout relativeHeader;
    HeaderManager headermanager;
    ImageView back;
    Context mContext = this;
    ArrayList<WorksheetSubjectModel> subjectList;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(AppUtilityMethods.isNetworkConnected(mContext)) {
            getList(URL_GET_WORKSHEET_SUBJECT_LIST);
        }else{
                ...
                try {
                    ...
                    if (response_code.equalsIgnoreCase(RESPONSE_SUCCESS)) {
                        ...
                        if (status_code.equalsIgnoreCase(STATUSCODE_SUCCESS)) {
                            ...
                            JSONArray subjectArray = secobj.getJSONArray(JTAG_SUBJECT_ARRAY);
                            ...
                            if (subjectArray.length() > 0) {
                                ...
                                for (int i = 0; i < subjectArray.length(); i++) {
                                    ...
                                    WorksheetSubjectModel subjectModel=new WorksheetSubjectModel();
                                    subjectModel.setId(sObject.optString(JTAG_ID));
                                    subjectModel.setSubjectName(sObject.optString(JTAG_SUBJECT_NAME));
                                ...
                                WorksheetSubjectListAdapter mSubjectAdapter=new WorksheetSubjectListAdapter(mContext,subjectList);
    ...
    private void initUI() {
        ...
        headermanager = new HeaderManager(WorkSheetSubjectListActivity.this, "Worksheet");
        if (AppPreferenceManager.getSchoolSelection(mContext).equals("ISG")) {
        ...
        messageList.addOnItemTouchListener(new RecyclerItemListener(getApplicationContext(), messageList,
                new RecyclerItemListener.RecyclerTouchListener() {
                    public void onClickItem(View v, int position) {
                        ...
                        Intent intent=new Intent(WorkSheetSubjectListActivity.this,WorksheetListActivity.class);
                        intent.putExtra("subjectId",subjectList.get(position).getId());
...
//        stopDisconnectTimer();
    ...
    @Override protected void onRestart() { super.onRestart();
        ...
        if (AppPreferenceManager.getUserId(mContext).equalsIgnoreCase("")) {
            Intent mIntent = new Intent(WorkSheetSubjectListActivity.this, LoginActivity.class);
            mIntent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_NEW_TASK);
            startActivity(mIntent);
            finish();
        }
