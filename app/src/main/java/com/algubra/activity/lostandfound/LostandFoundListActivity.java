...
import com.algubra.activity.lostandfound.adapter.LostandFoundListAdapter;
 ...
 */
public class LostandFoundListActivity extends  Activity implements JsonTagConstants,StausCodes,URLConstants {
    ...
    ArrayList<LostandFoundModel>   listLostFound = new ArrayList<>();
    Dialog dialog;
    Dialog dialog1;
    ...
    private void initUI() {
        ...
        headermanager = new HeaderManager(LostandFoundListActivity.this, "Lost & Found");
        ...
        //addItem = headermanager.getRightButton();
        ...
        messageList.addOnItemTouchListener(new RecyclerItemListener(getApplicationContext(), messageList,
                new RecyclerItemListener.RecyclerTouchListener() {
                    public void onClickItem(View v, int position) {
                        stopDisconnectTimer();
                        Intent intent = new Intent(LostandFoundListActivity.this, LostandFoundDetailActivity.class);
                        intent.putExtra("array", listLostFound);
    ...
    /*addItem.setOnClickListener(new View.OnClickListener() {
        @Override
        public void onClick(View v) {
            Intent intent = new Intent(LostandFoundListActivity.this, PostLostandFoundActivity.class);
            startActivity(intent);
            finish();
        }
    });*/
                ...
                try {
                    ...
                    if (response_code.equalsIgnoreCase(RESPONSE_SUCCESS)) {
                        ...
                        if (status_code.equalsIgnoreCase(STATUSCODE_SUCCESS)) {
                            ...
                            JSONArray staffArray = secobj.getJSONArray(JTAG_LOSTANDFOUND);
                            ...
                            if (staffArray.length() > 0) {
                                for (int i = 0; i < staffArray.length(); i++) {
                                    ...
                                    lostandFoundModel.setId(jsonObject.optString(JTAG_ID));
                                    lostandFoundModel.setUserId(jsonObject.optString(JTAG_USER_ID));
                                    lostandFoundModel.setDescription(jsonObject.optString(JTAG_DESCRIPTION));
                                    lostandFoundModel.setTitle(jsonObject.optString(JTAG_TITLE));
                                    lostandFoundModel.setContact_person_email(jsonObject.optString(JTAG_CONTACT_P_EMAIL));
                                    lostandFoundModel.setContact_person_phone(jsonObject.optString(JTAG_CONTACT_P_NO));
                                    lostandFoundModel.setContact_person_name(jsonObject.optString(JTAG_CONTACT_P_NAME));
                                    lostandFoundModel.setPosted_user(jsonObject.optString(JTAG_POSTED_BY));
                                    lostandFoundModel.setPosted_user_contact(jsonObject.optString(JTAG_POSTED_PHONE));
                                    lostandFoundModel.setPosted_user_email(jsonObject.optString(JTAG_POSTED_EMAIL));
                                    lostandFoundModel.setLostImage(jsonObject.optString(JTAG_IMAGE));
                                    listLostFound.add(lostandFoundModel);
                                }
                            } else {
                            ...
                            LostandFoundListAdapter lostandFoundListAdapter = new LostandFoundListAdapter(mContext, listLostFound);
                            messageList.setAdapter(lostandFoundListAdapter);
   ...
   /* public static final long DISCONNECT_TIMEOUT = 300000;; // 5 min = 5 * 60 * 1000 ms
    ...
    private Runnable disconnectCallback = new Runnable() {
        ...
        public void run() {
            ...
            AppPreferenceManager.setIsGuest(LostandFoundListActivity.this, false);
            AppPreferenceManager.setUserId(LostandFoundListActivity.this, "");
            AppPreferenceManager.setIsUserAlreadyLoggedIn(LostandFoundListActivity.this, false);
            AppPreferenceManager.setSchoolSelection(LostandFoundListActivity.this, "ISG");
            if (AppUtilityMethods.isAppInForeground(mContext)) {

                Intent mIntent = new Intent(LostandFoundListActivity.this, LoginActivity.class);
